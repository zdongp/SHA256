...
void *sha256_read_ctx (const struct sha256_ctx *ctx, void *resbuf)
{
  ...
  for (i = 0; i < 8; i++)
    set_uint32 (r + i * sizeof ctx->state[0], ctx->state[i]);//REDU(SWAP (ctx->state[i]))
...
//File hash user interface
void HashFile(char const *filepath, uint32 *hash)
{
  FILE *fp;
  int i;
